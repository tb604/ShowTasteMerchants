 ...
 * 文件名称: CTCRestaurantMealingFoodOrderViewCell.m
 * 文件标识:
 * 摘要描述:
 *
 * 当前版本:
 * 作者姓名: 唐斌
 * 创建日期: 2016/10/18 09:43
 * 完成日期:
 *
 * 取代版本:
 * 作者姓名: 输入作者(或修改者)名字
...
@interface CTCRestaurantMealingFoodOrderViewCell ()
{
...
@implementation CTCRestaurantMealingFoodOrderViewCell
...
- (void)initWithAmountLabel
{
    if (!_amountLabel)
    {
        ...
        CGRect frame = CGRectMake(_thumalImgView.left - 5 - width, 10, width, 20);
...
- (void)initWithNameLabel
{
    if (!_nameLabel)
    {
        CGRect frame = CGRectMake(_thanImgView.right + 5, 10, _numberLabel.left - 5 - _thanImgView.right - 2, 20);
