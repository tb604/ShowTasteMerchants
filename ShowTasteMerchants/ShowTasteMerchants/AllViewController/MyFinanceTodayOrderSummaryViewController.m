//
//  MyFinanceTodayOrderSummaryViewController.m
//  ChefDating
//
...
#import "MyFinanceTodayOrderSummaryViewController.h"
#import "LocalCommon.h"
#import "UserLoginStateObject.h"

@interface MyFinanceTodayOrderSummaryViewController ()
{
    BOOL _isLoadSuccess;
}
@end

@implementation MyFinanceTodayOrderSummaryViewController
...
- (void)initWithVar
{
    ...
    //http://182.254.132.142:8082/order-report/day-order?shop_id=1&date=20160912
    
//    REQUESTBASICURL
    NSString *strDate = [_date stringByReplacingOccurrencesOfString:@"-" withString:@""];
    if (kIsProductioinEnvironment == 1)
    {// 生产环境
        self.url = [NSString stringWithFormat:@"%@order-report/day-order?shop_id=%d&date=%@", H5ROOTURL, (int)[UserLoginStateObject getCurrentShopId], strDate];
    }
    else
    {// 测试环境
