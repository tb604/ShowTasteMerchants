...
- (UIBezierPath *)graphPathFromPoints
{
    ...
    for (NSInteger i=0; i<points.count; i++)
    {
        ...
        MPPButton *button = [MPPButton buttonWithType:UIButtonTypeCustom tappableAreaOffset:UIOffsetMake(25, 25)];
...
{
//    debugLog(@"width=%.2f, scwi=%.2f", self.frame.size.width, [[UIScreen mainScreen] screenWidth]);
    CGFloat space = (self.frame.size.width)/(points.count);
    CGPoint point = CGPointMake(space/2+(space)*index,self.height-((self.height-PADDING*2)*[[points objectAtIndex:index] floatValue]+PADDING));

//    CGFloat space = (self.frame.size.width)/(points.count+1);
//    CGPoint point = CGPointMake(space+(space)*index,self.height-((self.height-PADDING*2)*[[points objectAtIndex:index] floatValue]+PADDING));
//    debugLogPoint(point);
    return point;
}
...
- (void)animate
{
    ...
    for (NSInteger i=0;i<points.count;i++)
    {
        ...
        MPPButton *button=[MPPButton buttonWithType:UIButtonTypeCustom];
        [button setBackgroundColor:self.graphColor];
        button.layer.cornerRadius=3;
        button.frame=CGRectMake(0, 0, 6, 6);
        button.center=point;
