...
@implementation NSString (TYZAdd)
...
{
    return [self sizeForFont:font size:CGSizeMake(HUGE, height) mode:NSLineBreakByWordWrapping].width;
}
 ...
 *  统计字符数
 */
int tyzStrLength(char const * const str)
{
    int i = 0;
    int len = 0;
    while (str[i])
...
- (int)getCharacterLen
{
    return tyzStrLength([self UTF8String]);
}
