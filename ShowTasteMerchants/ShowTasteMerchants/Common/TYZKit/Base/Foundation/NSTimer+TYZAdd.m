...
TYZSYNTH_DUMMY_CLASS(NSTimer_TYZAdd)

@implementation NSTimer (TYZAdd)
/**
...
{
    return [NSTimer scheduledTimerWithTimeInterval:seconds target:self selector:@selector(_tyz_ExecBlock:) userInfo:[block copy] repeats:repeats];
}
...
{
    return [NSTimer timerWithTimeInterval:seconds target:self selector:@selector(_tyz_ExecBlock:) userInfo:[block copy] repeats:repeats];
}

+ (void)_tyz_ExecBlock:(NSTimer *)timer
{
    if ([timer userInfo])
    {
