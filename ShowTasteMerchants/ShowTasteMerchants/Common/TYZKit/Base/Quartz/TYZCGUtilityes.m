...
CGContextRef TYZCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale)
{
...
CGContextRef TYZCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale)
{
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform TYZCGAffineTransformGetFromViews(UIView *from, UIView *to)
{
    ...
    return TYZCGAffineTransformGetFromPoints(before, after);
}
