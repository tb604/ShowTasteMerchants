...
TYZSYNTH_DUMMY_CLASS(UIDevice_TYZAdd)


//@interface TYZSYNTH_DUMMY_CLASS_UIDevice_TYZAdd : NSObject
//@end
//@implementation TYZSYNTH_DUMMY_CLASS_UIDevice_TYZAdd
//@end

@implementation UIDevice (TYZAdd)
...
typedef struct
{
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} tyz_net_interface_counter;


static uint64_t tyz_net_counter_add(uint64_t counter, uint64_t bytes)
{
...
static uint64_t tyz_net_counter_get_by_type(tyz_net_interface_counter *counter, TYZNetworkTrafficType type)
{
    uint64_t bytes = 0;
    if (type & TYZNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & TYZNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & TYZNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & TYZNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & TYZNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & TYZNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static tyz_net_interface_counter tyz_get_net_interface_counter()
{
    ...
    tyz_net_interface_counter counter = {0};
        ...
        while (cursor)
        {
            if (cursor->ifa_addr->sa_family == AF_LINK)
            {
                ...
                if (name)
                {
                    ...
                    counter_in = tyz_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = tyz_net_counter_add(counter_out, data->ifi_obytes);
...
{
    tyz_net_interface_counter counter = tyz_get_net_interface_counter();
    return tyz_net_counter_get_by_type(&counter, types);
}
