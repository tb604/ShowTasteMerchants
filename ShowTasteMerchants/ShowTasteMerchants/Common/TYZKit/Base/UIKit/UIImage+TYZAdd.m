...
TYZSYNTH_DUMMY_CLASS(UIImage_TYZAdd)

/**
 *  得到每一贴gif图片的动画是的时间(单位：秒)
 *
 *  @param source
 *  @param index  第几贴
 *
 *  @return 动画的秒数
 */
static NSTimeInterval _tyz_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index)
{
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale
{
    ...
    // 得到原始图片的数量，主要是gif，应为一个gif是多个图片组成的
    size_t count = CGImageSourceGetCount(source);
//    debugLog(@"count=%zd", count);
    if (count <= 1)
    {// 表示不是gif图片
    ...
    NSTimeInterval totalTime = 0; // 总动画需要的时间
    NSUInteger totalFrame = 0;
    NSUInteger gcdFrame = 0;
    for (size_t i = 0; i < count; i++)
    {
        // 得到一帖动画的秒数
        NSTimeInterval delay = _tyz_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
        NSInteger frame = lrint(delay / oneFrameTime);
//        debugLog(@"frame=%d", (int)frame);
        if (frame < 1)
        {
            frame = 1;
        }
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            CFRelease(source), source = NULL;
            CFRelease(imageRef), imageRef = NULL;
            return nil;
        }
...
{
    return [self _tyz_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size
{
    return [self _tyz_imageWithPDF:dataOrPath resize:YES size:size];
}
 ...
 *  根据pdf文件，返回图片
 *
 *  @param dataOrPath pdf文件路径或者data
 *  @param resize     是否调整
 *  @param size       图片大小
 *
 *  @return 图片
 */
+ (UIImage *)_tyz_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size
{
...
{
    CGRect drawRect = TYZCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)_tyz_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical
{
...
{
    return [self _tyz_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical
{
    return [self _tyz_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal
{
    return [self _tyz_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+TYZAdd error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+TYZAdd error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+TYZAdd error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _tyz_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+TYZAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+TYZAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_tyz_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _tyz_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void _tyz_cleanupBuffer(void *userData, void *buf_data)
{
    free(buf_data);
}

// Helper function to add tint and mask.
- (UIImage *)_tyz_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
...
+ (UIImage *)imageWithContentsOfFileName:(NSString *)imageName
{
    // [[NSBundle mainBundle] pathForScaledResource:@"space_bg.png" ofType:nil];
//    NSString *path = [[NSBundle mainBundle] pathForResource:imageName ofType:nil];
    NSString *path = [[NSBundle mainBundle] pathForScaledResource:imageName ofType:nil];
//    debugLog(@"path=%@", path);
