...
/**
 TYZMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 TYZMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory
 ...
 The time of `Access Methods` in TYZMemoryCache is typically in constant time (O(1)).
 */

@interface TYZMemoryCache : NSObject
...
/** The number of objects in the cache (read-only)(缓存中对象的总数量) */
@property (readonly) NSUInteger totalCount;

/** The total cost of objects in the cache (read-only).(缓存占用的内存总的大小) */
...
/**
 The maximum number of objects the cache should hold.(缓存对象的个数，默认最大值，就是没有限制)
...
/**
 The maximum total cost that the cache can hold before it starts evicting objects.(限制占用缓存内存的大小)
...
/**
 The maximum expiry time of objects in cache.(在缓存中的时间限制)
 ...
 */
@property (copy) void(^didReceiveMemoryWarningBlock)(TYZMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (copy) void(^didEnterBackgroundBlock)(TYZMemoryCache *cache);
