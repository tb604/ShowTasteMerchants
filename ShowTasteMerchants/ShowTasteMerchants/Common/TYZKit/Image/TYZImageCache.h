...
@class TYZMemoryCache, TYZDiskCache;
 ...
 *  Image cache type
 */
typedef NS_OPTIONS(NSUInteger, TYZImageCacheType)
{
    TYZImageCacheTypeNone = 0, ///< No value.
    TYZImageCacheTypeMemory = 1 << 0, ///< Get/store image with memory cache.
    TYZImageCacheTypeDisk = 1 << 1, ///< Get/store image with disk cachel
    TYZImageCacheTypeAll = TYZImageCacheTypeMemory | TYZImageCacheTypeDisk,///< Get store image with both memory cache and disk cache.
};
 ...
 */
@interface TYZImageCache : NSObject
...
@property (strong, readonly) TYZMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) TYZDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(TYZImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(TYZImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(TYZImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, TYZImageCacheType type))block;
