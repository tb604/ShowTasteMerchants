 ...
 *
 * 项目名称: TYZScan
 * 文件名称: SubTYZScanViewController.m
 * 文件标识:
 * 摘要描述:
 *
 * 当前版本:
 * 作者姓名: 唐斌
 * 创建日期: 2016/11/1 09:29
 * 完成日期:
 *
 * 取代版本:
 * 作者姓名: 输入作者(或修改者)名字
...
#import "MyQRViewController.h" // shengcheng 二维码
#import "ScanResultViewController.h" // 结果
#import "TYZScanResult.h"
#import "TYZScanWrapper.h"
#import "TYZScanVideoZoomView.h" // 扫描视频缩放视图
#import "TYZAlertAction.h"

@interface SubTYZScanViewController ()

@property (nonatomic, strong) TYZScanVideoZoomView *zoomView;

@end

@implementation SubTYZScanViewController
...
- (TYZScanVideoZoomView *)zoomView
{
    if (!_zoomView)
    {
        ...
        _zoomView = [[TYZScanVideoZoomView alloc]initWithFrame:CGRectMake((CGRectGetWidth(self.view.frame)-zoomw)/2, YMaxRetangle + 40, zoomw, 18)];
...
{
    [TYZAlertAction showAlertWithTitle:@"提示" msg:str chooseBlock:nil buttonsStatement:@"知道了",nil];
}
...
- (void)popAlertMsgWithScanResult:(NSString*)strResult
{
    ...
    __weak __typeof(self) weakSelf = self;
    [TYZAlertAction showAlertWithTitle:@"扫码内容" msg:strResult chooseBlock:^(NSInteger buttonIdx) {
