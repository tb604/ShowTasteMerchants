...
@interface TYZScanView ()

/// 扫码区域各种参数
@property (nonatomic, strong) TYZScanViewStyle *viewStyle;
...
/// 线条扫码动画封装
@property (nonatomic, strong) TYZScanLineAnimation *scanLineAnimation;

/// 网格扫码动画封装
@property (nonatomic, strong)TYZScanNetAnimation *scanNetAnimation;
...
{
//    NSLog(@"animationStle=%d", _viewStyle.anmiationStyle);
    switch (_viewStyle.anmiationStyle)
    {
        case TYZScanViewAnimationStyle_LineMove:
        {// 线条上下移动
//            NSLog(@"线条上下移动");
            //线动画
            ...
            {
//                NSLog(@"创建对象");
                self.scanLineAnimation = [[TYZScanLineAnimation alloc] init];
            }
            ...
            break;
        case TYZScanViewAnimationStyle_NetGrid:
        {
            //网格动画
            if (!_scanNetAnimation)
                self.scanNetAnimation = [[TYZScanNetAnimation alloc] init];
            [_scanNetAnimation startAnimatingWithRect:_scanRetangleRect
                                               inView:self
                                                image:_viewStyle.animationImage];
        }
            break;
        case TYZScanViewAnimationStyle_LineStill:
        {
            if (!_scanLineStill)
            {
...
- (void)drawScanRect
{
    ...
    {
        case TYZScanViewPhotoframeAngleStyle_Outer:
        {
            diffAngle = linewidthAngle/3;//框外面4个角，与框紧密联系在一起
        }
            break;
        case TYZScanViewPhotoframeAngleStyle_On:
        {
            diffAngle = 0;
        }
            break;
        case TYZScanViewPhotoframeAngleStyle_Inner:
        {
