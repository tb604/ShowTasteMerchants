 ...
 *
 * 项目名称: TYZScan
 * 文件名称: TYZScanWrapper.m
 * 文件标识:
 * 摘要描述:
 *
 * 当前版本:
 * 作者姓名: 唐斌
 * 创建日期: 2016/10/29 11:28
 * 完成日期:
 *
 * 取代版本:
 * 作者姓名: 输入作者(或修改者)名字
...
@interface TYZScanWrapper ()

@property (nonatomic, strong) TYZScanNative *scanNativeObj;
...
- (instancetype)initWithPreView:(UIView *)preView ArrayObjectType:(NSArray *)arrayBarCodeType cropRect:(CGRect)cropRect success:(void (^)(NSArray<TYZScanResult *> *))blockScanResult
{
    if (self = [super init])
    {
        ...
        if ([TYZScanWrapper isSysIos7Later])
        {
            NSLog(@"ios 7.0以后");
            _scanNativeObj = [[TYZScanNative alloc] initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray<TYZScanResult *> *array) {
                if (blockScanResult)
                {
...
- (void)changeScanObjType:(NSArray *)barCodeType
{
    if ([TYZScanWrapper isSysIos7Later])
    {
        [_scanNativeObj changeScanType:barCodeType];
    }
}

// 识别图片上的二维码
+ (void)recognizeImage:(UIImage *)image success:(void (^)(NSArray<TYZScanResult *> *))block
{
...
+ (UIImage *)createQRWithString:(NSString *)str size:(CGSize)size
{
    if ([TYZScanWrapper isSysIos7Later])
    {
        ...
        return [TYZScanWrapper createNonInterpolatedUIImageFormCIImage:[TYZScanWrapper createQRForString:str] withSize:size.width];
    }
    else
        return nil;
...
//    ZXBarcodeFormat zxformat = [LBXScanWrapper convertCodeFomratToZXBarcodeFormat:format];
    ...
    return  nil;//[ZXingWrapper createCodeWithString:str size:size CodeFomart:zxformat];
}
