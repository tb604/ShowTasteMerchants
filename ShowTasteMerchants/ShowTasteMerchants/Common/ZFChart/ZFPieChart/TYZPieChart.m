...
- (void)commonInit
{
    ...
    _radius = self.height/3;//_maxRadius * 0.25;
    _lineWidth = _radius;
    _totalDuration = 0.75f;
    _startAngle = ZFRadian(-90);
    _extendLength = 20.f;
    _originHeight = self.frame.size.height;
//    [XWPieChartTitleView getWithWidth]
    if (kiPhone4 || kiPhone5)
    {
        _pieCenter = CGPointMake(30 + _radius , self.height/2);
    }
    else
    {
        _pieCenter = CGPointMake(35 + _radius , self.height/2);//CGPointMake(self.center.x, CGRectGetHeight(self.titleLabel.frame) + NAVIGATIONBAR_HEIGHT + _radius);
    }
    _isShowDetail = YES;
    _isShowPercent = YES;
...
- (void)addUI
{
    ...
    NSString *name = _nameArray[1];
    UIColor *color = _colorArray[1];
    XWPieChartTitleView *middleView = [[XWPieChartTitleView alloc] initWithFrame:CGRectMake(0, (self.height-20)/2, [XWPieChartTitleView getWithWidth], 20)];
//    middleView.backgroundColor = [UIColor lightGrayColor];
    middleView.right = self.width - 10;
    [middleView updateWithName:name color:color];
    [self addSubview:middleView];
    
    name = _nameArray[0];
    color = _colorArray[0];
    CGRect frame = middleView.frame;
    XWPieChartTitleView *topView = [[XWPieChartTitleView alloc] initWithFrame:frame];
//    topView.backgroundColor = [UIColor lightGrayColor];
    topView.bottom = middleView.top;
    [topView updateWithName:name color:color];
    [self addSubview:topView];
    
    name = _nameArray[2];
    color = _colorArray[2];
    frame = middleView.frame;
    XWPieChartTitleView *bottomView = [[XWPieChartTitleView alloc] initWithFrame:frame];
//    bottomView.backgroundColor = [UIColor lightGrayColor];
    bottomView.top = middleView.bottom;
    [bottomView updateWithName:name color:color];
...
//    debugLogPoint(self.centerPoint);
    //记录开始角度和结束角度
...
//    debugLog(@"lineWith=%.2f", _lineWidth);
...
//    debugLog(@"frame=%@", NSStringFromCGRect(shapeLayer.frame));
