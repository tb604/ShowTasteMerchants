...
@property (nonatomic, strong) XMLHelper *xml;
@end
...
- (NSString *)sendPrepay:(NSMutableDictionary *)prePayParams
{
    ...
    self.xml  = [[XMLHelper alloc] init];//[[XMLHelper alloc] autorelease];
    
    //开始解析
    [_xml startParse:res];
    
    NSMutableDictionary *resParams = [_xml getDict];
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    packageParams[@"mch_id"] = mchid; //商户号
//    [packageParams setObject: @"APP-001"        forKey:@"device_info"]; //支付设备号或门店号
    packageParams[@"nonce_str"] = noncestr; //随机串
    packageParams[@"trade_type"] = @"APP"; //支付类型，固定为APP
    packageParams[@"body"] = ordername; //订单描述，展示给用户
    packageParams[@"notify_url"] = notifyurl; //支付结果异步通知
    packageParams[@"out_trade_no"] = tradeno; //商户订单号
    NSString *ip = [WXUtil getIPAddress:YES];
//    NSLog(@"ip=%@", ip);
    packageParams[@"spbill_create_ip"] = ip; //发器支付的机器ip
    NSString *price = [NSString stringWithFormat:@"%d", (int)(orderprice * 100)];
//    NSLog(@"price=%@", price);
    packageParams[@"total_fee"] = price; //订单金额，单位为分
